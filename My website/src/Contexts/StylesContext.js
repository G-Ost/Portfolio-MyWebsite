...
function StylesContext(props) {
  ...
  const [sizeUnit, setSizeUnit] = React.useState(
    window.matchMedia(" (max-height: 600px) ").matches ? 140 : 350
  );

  React.useEffect(() => {
    window.addEventListener("orientationchange", () => {
      setSizeUnit(sizeUnit === 180 ? 350 : 180);
    });
  ...
  let menuOptionStyle = {
    ...
    width: sizeUnit / 2.5,
    pointerEvents: "auto",
    backgroundColor: "rgb(300,100,100)",
  };
  ...
  function menuOptionStart(optionElement, wrapperElement, newSizeUnit) {
    ...
    setTimeout(() => {
      wrapperElement.current.style.visibility = "visible";
    }, 400);
  }
